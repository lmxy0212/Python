import random
from Lecture.UnsortedArrayMap import UnsortedArrayMap
...
class ChainingHashTableMap:
    ...
    def __setitem__(self, key, value):
        ...
        if (new_size > old_size):  # check if new item is added
            self.n += 1
        if (self.n > self.N):
    ...
    def __iter__(self):  # yield KEYYYYYYYYY!!! map[i]
    ...
    def rehash(self, new_size):
        ...
        for key in self:
            value = self[key]
            # print(value)
            old.append((key, value))  # store (key,val) to lst
        self.table = [None] * new_size
        self.n = 0
        self.N = new_size
        for (key, value) in old:  # put into new hash table
            self[key] = value