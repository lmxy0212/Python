...
def find_min_abs_difference(bst):
    ...
    result = find_min_helper(bst.root)[2]
    return result

def find_min_helper(curr):
    if curr is None:
        return (-math.inf,math.inf,math.inf)
    else:
        left = find_min_helper(curr.left)
        # print(curr.item.key,left)
        right = find_min_helper(curr.right)
        # print(curr.item.key,right)
        max_val = max(left[0], right[0])
        min_val = min(left[1], right[1])
        diff = min(abs(curr.item.key - max_val),abs(curr.item.key - min_val),left[2],right[2])
        if curr.item.key < min_val:
            min_val = curr.item.key
        if curr.item.key > max_val:
            max_val = curr.item.key
        return (max_val,min_val,diff)

def main():
    bstree = BinarySearchTreeMap()
    bstree[15] = None
    bstree[10] = None
    bstree[20] = None
    bstree[5] = None
    ...
    bstree[1] = None
    bstree[8] = None
    for i in bstree.inorder():
        print(i.item.key, end=' ')
    print()
    print(find_min_abs_difference(bstree))
main()